...
function _ProcessBindDown(self,key)
	(self["_dd_keybinds_in"][key] or NULL_FUNC)(self)
end
function _ProcessBindUp(self,key)
...
function PLAYER:AddBind(key,func,state)
	...
	if self[state][key] then ErrorNoHalt("DDCORD:Cant add bind to [" .. _GetKeyName(key) .. "], key already occupied!") return end
	self[state][key] = func
end
...
hook.Add("PlayerInitialSpawn","DD_CORD_PlayerInitialSpawn",function(ply)
	ply["_dd_keys_state"] = {}
	ply["_dd_keybinds_in"] = {}
	ply["_dd_keybinds_out"] = {}
end)
hook.Add("PlayerButtonDown","DD_CORD_PlayerKeyDown",function(ply,key)
	ply["_dd_keys_state"][key] = true
	_ProcessBindDown(ply,key)
end)
hook.Add("PlayerButtonUp","DD_CORD_PlayerKeyUp",function(ply,key)
